              ...
              <div className="relative z-10 flex flex-col items-center">
                <div className="flex -space-x-2 mb-3">
                  <div className="w-12 h-12 rounded-full bg-white/10 border border-white/30 flex items-center justify-center z-10">
                    <img src="https://futbolinclusivo.org.ar/app/uploads/2022/03/satlogo.png" alt="SAT" width={32} height={32} className="object-contain" />
                  </div>
              ...
              <div className="grid gap-5 md:gap-8">
                {recentNews.map((news, idx) => (
                  <motion.div 
                    ...
                    key={idx}
                  >
                    <Link href="/novedades" className="group flex flex-col sm:flex-row gap-4 bg-white/[0.02] rounded-xl md:rounded-2xl overflow-hidden border border-white/5 transition-all hover:bg-white/[0.05] hover:border-white/20 shadow-lg p-3 md:p-5">
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  {[
                    ...
                    <div key={i} className="p-3 md:p-4 bg-white/5 rounded-xl border border-white/10 text-center shadow-inner">
                      ...
                      <p className="text-[10px] md:text-xs text-white/50 font-bold uppercase tracking-wide">{f.dia}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-3 md:gap-4 mb-4 md:mb-6 relative z-10">
                {[
                  ...
                  <div key={i} className="flex justify-between items-center p-3 md:p-4 bg-gradient-to-r from-white/[0.05] to-transparent rounded-xl border border-white/10 hover:border-[#36b37e]/30 hover:from-[#36b37e]/10 transition-all duration-500 group/item">
                    <div className="flex items-center gap-3 md:gap-4 min-w-0">
                      <span className="text-lg md:text-xl shrink-0">{p.icon}</span>
                      <div className="min-w-0">
                        <p className="font-black text-sm md:text-base uppercase leading-tight text-white mb-0.5 truncate group-hover/item:text-[#36b37e] transition-colors">{p.name}</p>
                        <p className="text-[10px] md:text-xs text-white/50 font-bold uppercase tracking-wide">{p.team}</p>
                      </div>
                    </div>
                    <span className="bg-gradient-to-br from-[#36b37e] to-[#2da372] text-white font-black px-4 md:px-5 py-1.5 md:py-2 rounded-lg text-sm md:text-base shadow-[0_0_20px_rgba(54,179,126,0.3)] shrink-0 border border-white/20">{p.goals}</span>
                  </div>
                ))}
              </div>
